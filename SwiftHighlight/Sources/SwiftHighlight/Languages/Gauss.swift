...
/// gauss language definition
public func gaussLanguage(_ hljs: HighlightJS) -> Language {
    let lang = Language()
    lang.name = "GAUSS"
    ...
    lang.contains = [
        ...
        .mode({
            ...
            mode.keywords = .grouped([
                "keyword": "define definecs|10 undef ifdef ifndef iflight ifdllcall ifmac ifos2win ifunix else endif lineson linesoff srcfile srcline" as Any,
            ])
        ...
        .mode({
            let mode = Mode()
            mode.begin = .regex("\\bexternal (matrix|string|array|sparse matrix|struct|proc|keyword|fn)")
        ...
        .mode({
            ...
            mode.beginKeywords = "proc keyword"
            ...
            mode.contains = [
                .mode({
                    ...
                    mode.contains = [
                        ...
                        .mode({
                            let mode = Mode()
                            mode.begin = .regex("\\bstruct\\s+")
                            mode.end = .regex("\\s")
                            mode.keywords = .simple("struct")
        ...
        .mode({
            let mode = Mode()
            mode.end = .regex(";")
            mode.beginKeywords = "for threadfor"
            mode.relevance = 0
            mode.contains = [
                /* circular ref: __ref_1 */ .mode(Mode()),
                /* circular ref: __ref_2 */ .mode(Mode()),
                    ...
                    mode.contains = [
                        ...
                        .mode({
                            ...
                            mode.contains = [
                                .mode({
                                    let mode = Mode()
                                    mode.beginKeywords = "bool break call callexe checkinterrupt clear clearg closeall cls comlog compile continue create debug declare delete disable dlibrary dllcall do dos ed edit else elseif enable end endfor endif endp endo errorlog errorlogat expr external fn for format goto gosub graph if keyword let lib library line load loadarray loadexe loadf loadk loadm loadp loads loadx local locate loopnextindex lprint lpwidth lshow matrix msym ndpclex new open output outwidth plot plotsym pop prcsn print printdos proc push retp return rndcon rndmod rndmult rndseed run save saveall screen scroll setarray show sparse stop string struct system trace trap threadfor threadendfor threadbegin threadjoin threadstat threadend until use while winprint ne ge le gt lt and xor or not eq eqv"
                                    return mode
                                }()),
        ...
        .mode({
            ...
            mode.variants = [
                {
                    let mode = Mode()
                    mode.begin = .regex("[a-zA-Z_]\\w*\\.[a-zA-Z_]\\w*")
                    return mode
                }(),
                {
                    let mode = Mode()
...
/// Register gauss language
public extension HighlightJS {
    func registerGauss() {
        registerLanguage("gauss", definition: gaussLanguage)
    }
}
