...
public func excelLanguage(_ hljs: HighlightJS) -> Language {
    ...
    lang.contains = [
        ...
        .mode({
            let mode = Mode()
            mode.begin = .regex("\\b[A-Z]{1,2}\\d+\\b")
            mode.end = .regex("[^\\d]")
        ...
        .mode({
            let mode = Mode()
            mode.begin = .regex("[A-Z]{0,2}\\d*:[A-Z]{0,2}\\d*")
            mode.scope = .single("symbol")
            mode.relevance = 0
            return mode
        }()),
        ...
        .mode(CommonModes.COMMENT("\\bN\\(", "\\)")),
    ]

    return lang
}

/// Register excel language
public extension HighlightJS {
    func registerExcel() {
        registerLanguage("excel", definition: excelLanguage)
    }
}
